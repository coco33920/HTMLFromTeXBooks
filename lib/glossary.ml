...
let parse_glossary_entry entry =
  ...
  and s s = String.sub s 1 (String.length s - 1) in
  let not_comma_then_s t = t |> s |> String.trim in
  let a, q = read_desc "" (Utils.string_to_list entry) in
  let b, q2 = read_name "" q in
  let c, _ = read_def "" q2 in
  let a = [ a; b; c ] in
  let a =
    List.map
      (fun c ->
        Str.global_replace accolade_ouvrante_regexp ""
          (Str.global_replace accolade_fermante_regexp "" c))
      a
  in
  match a with
  | [ a; b; c ] ->
      let a, b, c =
        ( a,
          Str.global_replace name_regexp "" b,
          Str.global_replace desc_regexp "" c )
      in
...
let parse_glossaries file =
  ...
  if not (glossary_exists file) then []
    ...
    let list_of_entries =
      Str.split glossary_regexp string
      |> List.filter not_is_empty
      |> List.map (Str.global_replace newline_regexp "")
    in
    list_of_entries
...
let total_glossaries file =
  parse_glossaries file |> List.map parse_glossary_entry |> ignore

(** Transform the glossary to an HTML file *)
let prints_glossary () =
  if not @@ glossary_provided () then ""
  else
    ...
    let rec aux result entries =
      ...
      | (desc, (name, def)) :: q ->
          ...
          aux (result ^ l) q
    in
    let line = aux line (List.of_seq (Hashtbl.to_seq glossaries)) in
    line ^ "</div>\n"

(**Takes a glossary descriptor and returns its name and definition*)
let recognize_gls name =
  try Hashtbl.find glossaries name with _ -> (name, name)
