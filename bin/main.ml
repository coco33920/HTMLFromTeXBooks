...
let execute_command file outname start_chapter writing =
  Htmlfromtexbooks.Htmlgen.print_file_in_html ~min_chap:start_chapter
    ~write_before:writing file outname

let parse_filename file outname start_chapter writing =
  if not (Sys.file_exists file) then (
    Printf.printf "The %s file do not exists" file;
    exit 2)
...
let detect_a_file () =
  ...
  in
  match a with None -> "" | Some v -> (Sys.readdir ".").(v)

let msg =
  "htmlfromtex --input <file> --output <out_file> [| --name <name> | \
...
let _ =
  ...
  if !filename = "" then (
    ...
    Printf.printf "input filename has been automatically detected to be %s\n"
      !filename)
  else Printf.printf "input filename is %s\n" !filename;
  if !outname = "" then (
    let b = Filename.remove_extension !filename in
    outname := b ^ ".html";
    Printf.printf "output filename has been automatically generated to be %s\n"
      !outname)
  else Printf.printf "output filename is %s\n" !outname;
